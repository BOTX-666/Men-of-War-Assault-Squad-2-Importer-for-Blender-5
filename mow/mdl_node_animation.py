						...
						for prop in event.properties:
							...
							# Check if this is a quaternion rotation property
								...
								if bone_node.orientation_matrix:
									...
									fcurve_rw.keyframe_points.add(1)
									fcurve_rw.keyframe_points[-1].co = (float(frame.time), q.w)
									fcurve_rx.keyframe_points.add(1)
									fcurve_rx.keyframe_points[-1].co = (float(frame.time), q.x)
									fcurve_ry.keyframe_points.add(1)
									fcurve_ry.keyframe_points[-1].co = (float(frame.time), q.y)
									fcurve_rz.keyframe_points.add(1)
									fcurve_rz.keyframe_points[-1].co = (float(frame.time), q.z)
								else:
									fcurve_rw.keyframe_points.add(1)
									fcurve_rw.keyframe_points[-1].co = (float(frame.time), prop.w)
									fcurve_rx.keyframe_points.add(1)
									fcurve_rx.keyframe_points[-1].co = (float(frame.time), prop.x)
									fcurve_ry.keyframe_points.add(1)
									fcurve_ry.keyframe_points[-1].co = (float(frame.time), prop.y)
									fcurve_rz.keyframe_points.add(1)
									fcurve_rz.keyframe_points[-1].co = (float(frame.time), prop.z)
				...
				for frame in seq.anm.keyframes:
					...
					for event in frame.events:
						...
						for prop in event.properties:
							...
							if type(prop) == ANM_FRAME_POSITION:
								# Apply position transformation
								fcurve_lx.keyframe_points.add(1)
								fcurve_lx.keyframe_points[-1].co = (float(frame.time), prop.x)
								fcurve_ly.keyframe_points.add(1)
								fcurve_ly.keyframe_points[-1].co = (float(frame.time), prop.y)
								fcurve_lz.keyframe_points.add(1)
								fcurve_lz.keyframe_points[-1].co = (float(frame.time), prop.z)
							# Check if this is a quaternion rotation property
							elif type(prop) == ANM_FRAME_QUATERNION:
								# Apply rotation transformation
								fcurve_rw.keyframe_points.add(1)
