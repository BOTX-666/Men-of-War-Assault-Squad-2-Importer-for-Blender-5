...
class MDL_NODE_VOLUME(MDL_NODE):
	...
	def build_blender_scene(self, blender_context):
		...
		try:
			blender_context.scene.collection.objects.link(blender_object)
		except (RuntimeError):
			pass
		#blender_context.scene.collection.objects.link(blender_object)

		# Move to a separate collection (replaces layers)
		vol_collection = bpy.data.collections.get("Volumes")
		if not vol_collection:
			vol_collection = bpy.data.collections.new("Volumes")
			blender_context.scene.collection.children.link(vol_collection)
		vol_collection.objects.link(blender_object)
		if blender_object.name in bpy.context.scene.collection.objects:
